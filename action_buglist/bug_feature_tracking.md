...
1. [open] [bug#00001] [function] **IBKR API 連線逾時與 Client ID 衝突處理**
   * **問題描述**：當多個 Client 連接同一個 TWS/Gateway 時，預設 Client ID 衝突會導致連線失敗且無明確警告。
   * **root cause**：
   * **solution**：
   * **fixed by**：

2. [open] [bug#00002] [function] **Firstrade CSV 匯入欄位變動相容性**
   * **問題描述**：若 Firstrade 匯出的 CSV 標頭欄位順序或名稱微調，會導致 `parse_positions_csv` 發生 KeyError。
   * **root cause**：
   * **solution**：
   * **fixed by**：

3. [open] [bug#00003] [UI] **ASCII 歷史淨值折線圖在窄版終端機的標籤折行**
   * **問題描述**：當終端機寬度小於 80 字元時，`history` 繪製的折線圖 Y 軸價格與 X 軸日期標籤容易錯位或折行。
   * **root cause**：
   * **solution**：
   * **fixed by**：

4. [open] [bug#00004] [newfeature] **基準貨幣 TWD ⇄ USD 切換支援即時匯率自動緩存**
...
6. [open] [bug#00010] [function] **選擇權持倉參數、自訂券商帳戶與持倉修改功能之優化**
...
7. [open] [bug#00014] [newfeature] **進階選擇權追蹤與 Black-Scholes 希臘字母監控 (Advanced Option Metrics & Greeks)**
   * **問題描述**：對於選擇權持倉，除了基礎參數外，缺乏希臘字母（Delta, Gamma, Theta, Vega）的估算，難以監控時間值衰減 (Theta Decay) 或價內外狀態 (ITM/OTM)。
   * **root cause**：
   * **solution**：
   * **fixed by**：

8. [open] [bug#00015] [newfeature] **多資產整合擴充（現金與加密貨幣錢包支援） (Multi-Asset Class Support: Cash & Crypto)**
   * **問題描述**：系統定位為統合性資產整合追蹤，但目前僅限於證券與期權。應擴充支援非證券的固定資產/現金科目（如銀行存款、數位穩定幣）手動登錄，以及加密貨幣公鏈餘額與交易所 API 自動同步。
   * **root cause**：
   * **solution**：
   * **fixed by**：

9. [open] [bug#00016] [newfeature] **大盤指數基準對比與時間/資金加權報酬率計算 (Portfolio Benchmarking & TWR/IRR Performance)**
...
10. [open] [bug#00017] [newfeature] **互動式全螢幕終端面板與本機/Webhook 警報系統 (Rich TUI Dashboard & Price Alerting)**
    * **問題描述**：目前的 CLI dashboard 為定時 `console.clear()` 刷新，易產生閃爍且無選單分頁。應使用 `Textual` 庫重構為互動式全螢幕終端介面 (TUI)；並實作價格與損益警報，當觸發時發送 macOS 系統通知或 Discord/Slack webhook。
    * **root cause**：
    * **solution**：
    * **fixed by**：

11. [open] [bug#00020] [newfeature] **純手動持倉管理與 API/CSV 匯入功能移除**
...
12. [open] [bug#00021] [newfeature] **功能選單重整與新增取消返回功能 (Action Menu Reorganization & Cancel Options)**
...
3. [closed] [bug#00008] [newfeature] **即時監控看板非阻塞式互動選單與子指令整合**
   * **問題描述**：即時更新看板會阻塞使用者輸入，使用者無法直接在畫面中執行任何操作。應實作非阻塞式輸入，允許使用者在看板中直接新增倉位、縮減倉位/登錄交易，且不中斷自動定時刷新報價的行為。
...
5. [closed] [bug#00011] [newfeature] **工具定位重構：移除下單/交易邏輯，補全持倉參數欄位**
...
6. [closed] [bug#00012] [function] **移除 Streamlit Web UI（dashboard.py），確保工具定位為純 CLI**
...
7. [closed] [bug#00013] [newfeature] **投資組合 Beta 權重與市場敏感度分析 (Portfolio Beta-Weighting & Analytics)**
...
8. [closed] [bug#00018] [UI] **歡迎畫面 Logo 拼寫錯誤與實體圖片 ASCII 轉換**
...
9. [closed] [bug#00019] [function] **歷史週頻市值 NaN 傳播與單一基準指數解析失效**
