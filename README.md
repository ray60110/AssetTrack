...
* **三張結論卡**（由上而下）：類股板塊共識 → 期權觀察（已發生樣態 + ATM 權利金貴賤，無股價漲跌預測） → 主動式 ETF 趨勢共識。
...
* **主動式 ETF 跨基金共識（快捷鍵 `6`）**：ETF universe 每日由市場 screener 動態建立，只納入 **AUM > US$5B** 且基金說明明確標示主動管理者，再依真實持股資產配置分類、分類內按 AUM 排序。於 14 天視窗比較逐日真實快照，部位級列出股票買入／賣出 ETF 數、總金額與整體多空；方向需**真實股數變化與權重變化兩訊號同向**才算。畫面另追蹤 Bridgewater、Citadel、Millennium、Elliott 的 SEC 13F 季度持股與相鄰申報期差分；13F 並非即時交易資料，Put/Call 亦不揭露履約價與到期日，因此不會被偽裝成精確期權時間區間訊號。
...
* Holdings、總資產、未實現損益、Portfolio Beta 與券商資產分布皆納入現金。資產分布另顯示 `cash_ratio`：低於 5% 為進攻、超過 20% 為防守，其餘為中性；並顯示總曝險、淨曝險與「總曝險 ÷ 總資產」的槓桿比例。一般股票／ETF 以 1x 計算，倍數與反向 ETF 依基金名稱自動辨識曝險倍數（新增／編輯時可手動覆寫），期權以 Delta × 合約數 × 合約乘數 × 標的現價換算等值曝險。任一必要報價不足時不輸出假比例。
...
| `7` | 期權觀察清單（預期波動、ATM 貴賤、Enter 看 IV−RV 走勢、持倉淨 Greeks） |
| `8` | 類股板塊分析 |
| `9` | 使用者績效追蹤：完整資產與 QQQ／VT 比較 |
...
| 主動式 ETF（`6`） | `j` 建議　`w` 觀察清單　`a` 研究全表　`h` 說明　`c` 清除快取　`s` SEC 身分　`Esc` 返回 |
